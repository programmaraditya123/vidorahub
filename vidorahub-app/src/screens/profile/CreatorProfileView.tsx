...
import type { Platform, Product, ProfileData, VideoItem } from '@/types';
import type { MainStackParamList } from '@/navigation/types';

export type CreatorProfileTab = 'videos' | 'store';
...
type CreatorProfileViewProps = {
  ...
  onDeleteVideo?: (videoId: string) => void | Promise<void>;
  videosEmptyText?: string;
...
export function CreatorProfileView({
  ...
  isOwner = false,
  showHeaderMenu,
  contentBottomPadding = spacing.huge,
  onOpenStudio,
  onOpenSettings,
  onDeleteVideo,
  ...
  const [tab, setTab] = useState<CreatorProfileTab>(initialTab);
  const [menuOpenFor, setMenuOpenFor] = useState<string | null>(null);
  const [confirmDeleteVideoId, setConfirmDeleteVideoId] = useState<string | null>(null);
  const [deleting, setDeleting] = useState(false);
  const [mobileMenuOpen, setMobileMenuOpen] = useState(false);

  const showOwnerControls = isOwner && Boolean(onDeleteVideo);
  const showMenu = showHeaderMenu ?? isOwner;
  ...
  const handleDelete = useCallback(async () => {
    if (!confirmDeleteVideoId || !onDeleteVideo) return;
    try {
      setDeleting(true);
      await onDeleteVideo(confirmDeleteVideoId);
    } finally {
      setDeleting(false);
      setConfirmDeleteVideoId(null);
    }
  }, [confirmDeleteVideoId, onDeleteVideo]);

  const openChannel = useCallback(
    () => {
      setMobileMenuOpen(false);
      navigation.navigate('Channel', { id: ownerId });
    },
    [navigation, ownerId],
  );
  ...
  return (
    ...
    >
      ...
      <View style={[styles.headerShell, { paddingTop: insets.top + spacing.sm }]}>
        <Pressable style={styles.logoWrap} onPress={() => navigation.navigate('Tabs', { screen: 'Home' })}>
      ...
      {showMenu && mobileMenuOpen ? (
        <>
          ...
          <View style={[styles.mobileMenu, { top: insets.top + HEADER_HEIGHT + spacing.xs }]}>
            ...
            />
            <MobileMenuItem icon="person-circle-outline" label="Channel" onPress={openChannel} />
            <MobileMenuItem icon="cloud-upload-outline" label="Upload" onPress={openUpload} />
            <MobileMenuItem icon="log-out-outline" label="Logout" onPress={handleLogout} danger />
          </View>
        </>
      ) : null}
            ...
            <View style={styles.masonry}>
              {filteredUploads.map((item) => (
                <Pressable key={item._id} style={styles.videoCard} onPress={() => handleVideoPress(item)}>
                  ...
                  <Pressable
                    style={[styles.menuDots, !showOwnerControls && styles.hidden]}
                    onPress={(e) => {
                      e.stopPropagation();
                  ...
                  {showOwnerControls && menuOpenFor === item._id ? (
                    <Pressable
              ...
              {!filteredUploads.length ? <Text style={styles.empty}>{videosEmptyText}</Text> : null}
            </View>
          ) : (
            <View style={styles.storeWrap}>
              {products.length ? (
                <ProductCard products={products} ownerId={ownerId} />
              ) : (
                <Text style={styles.empty}>{storeEmptyText}</Text>
              )}
            </View>
          )}
        </View>
        ...
        <View style={styles.sidebar}>
          <View style={styles.aboutCard}>
            <View style={styles.headerRow}>
              ...
              {isOwner && onOpenStudio ? (
                <Pressable onPress={onOpenStudio} style={styles.editBtn}>
                  <Text style={styles.editBtnText}>{profile.bio || connectItems.length ? 'Edit' : 'Add'}</Text>
                </Pressable>
              ) : null}
            </View>
            <Text style={styles.aboutText}>
              {profile.bio || (isOwner ? 'No bio added yet. Tell people about yourself.' : 'No bio added yet.')}
            </Text>
            ...
            <View style={styles.connectList}>
              {connectItems.length ? (
                ...
                <Pressable style={styles.connectItem} onPress={onOpenStudio}>
                ...
                <View style={styles.connectItem}>
                  <Text style={styles.connectItemLabel}>No social links added yet</Text>
                </View>
              )}
            </View>
          </View>
