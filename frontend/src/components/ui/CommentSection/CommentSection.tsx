...
export default function CommentsSection() {
  ...
  // Safe auth read after hydration
  useEffect(() => {
  ...
  const handlePost = async (parentId?: string) => {
    // Gate: unauthenticated users see modal instead
    if (!userSerialNumber) {
      setShowModal(true);
      return;
    }
    ...
    if (parentId) {
      setComments((prev) => insertReply(prev, parentId, optimisticComment));
      setReplyText("");
      setReplyTo(null);
    } else {
    ...
    try {
      ...
      setComments((prev) =>
        JSON.parse(
          JSON.stringify(prev).replace(optimisticComment._id, real._id)
        )
      );
    } catch {
      setComments((prev) =>
  ...
  const CommentItem = ({
    ...
    <div className={styles.commentRow} style={{ marginLeft: depth * 28 }}>
      <Image
      ...
      <div className={styles.wrapper}>
        <div className={styles.inputWrapper}>
          <textarea
            ...
            // Allow typing — gate is in handlePost, not here
          />
          <button className={styles.sendBtn} onClick={() => handlePost()}>
            <span>
